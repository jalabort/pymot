...
from pymot.external.munkres import munkres, max_cost_munkres
from pymot.external.munkres import iterative_munkres
...
def test_simple():
    ...
    for i in range(8):
        truth[7 - i, i] = True
    assert_array_equal(b, truth, 'simple 8x8 case failed, b=%s, truth=%s' % (str(b), str(truth)))
...
def test_big():
    ...
    for i in range(100):
        truth[99 - i, i] = True
    assert_array_equal(b, truth, 'simple 100x100 case failed, b=%s, truth=%s' % (str(b), str(truth)))
...
def test_max_cost():
    ...
    assert_array_equal(b, truth, 'basic 3x4 case failed\na=%s\ntruth=%s\ncost=%s\ntrue_cost=%s' % (str(a), str(truth), str(a[b]), str(a[truth])))
...
def test_iterative():
    ...
    assert_array_equal(a,truth, 'iterative assignment failed')
