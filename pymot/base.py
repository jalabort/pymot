...
class MOTEvaluation:
    ...
    def get_hypotheses_frame(self, timestamp):
        ...
        hypotheses_frames = [h for h in self.hypotheses_["frames"] if
                             hypothesis_frame_chronologically_close(h) ]
...
#            write_stderr_red("Warning:", "No hypothesis timestamp found for timestamp %f with sync delta %f" % (timestamp, self.sync_delta_))
    ...
    def evaluateFrame(self, frame):
        ...
        for groundtruth in groundtruths:
            LOG.info(BoundingBox.init_from_dic(groundtruth))

        LOG.info("Hypos:")
        for hypothesis in hypotheses:
            LOG.info(BoundingBox.init_from_dic(hypothesis))
...
#            print "DIFF Keep correspondence"
        ...
        for gt_id in self.mappings_.keys():
            ...
            # Get ground
            # truths with given ground truth id in current frame
            ...
            overlap = BoundingBox.init_from_dic(groundtruth[0]).iou(
                BoundingBox.init_from_dic(
                hypothesis[0]))
...
#                    print "DIFF Keep corr %s %s %.2f" % (groundtruth[0]["id"], hypothesis[0]["id"], Rect(groundtruth[0]).overlap(Rect(hypothesis[0])))
                listofprints.append("DIFF Keep corr %s %s %.2f" % (
                    ...
                    BoundingBox.init_from_dic(groundtruth[0]).iou(
                        BoundingBox.init_from_dic(
                        hypothesis[0]))))
        ...
        for i in range(len(groundtruths)):
            ...
            for j in range(len(hypotheses)):
                ...
                rect_groundtruth = BoundingBox.init_from_dic(groundtruth)
                rect_hypothesis = BoundingBox.init_from_dic(hypothesis)
                overlap = rect_groundtruth.iou(rect_hypothesis)
